...
train_data='CBIS-DDSM/transfer/DDSM_0227_level23_3000_h2_train/'
test_data='CBIS-DDSM/transfer/DDSM_0227_level23_3000_h2_test/'
train_dir = 'traindata/'
# Hyperparameter
growth_k = 12
...
with tf.Session() as sess:
    ckpt = tf.train.get_checkpoint_state('./model/class2')
   # print(ckpt.model_checkpoint_path)
    ...
    writer = tf.summary.FileWriter('./logs0407_class2', sess.graph)
    train_image= []
    train_label= []
    test_image = []
    test_label = []
   ...
   #	c = np.hstack(img)
    ...
    #	img = np.asarray([(np.array(xi)-mean)/std for xi in img])
	...
	test_accuracy=0
	   ...
	   # print(accuracy_rates)
    	...
    	if max_acc <= (test_accuracy/count):
	    max_acc = (test_accuracy/count)
	    print('save epoch, accuracy= ',max_acc)
	    saver.save(sess=sess, save_path='./model/dense_0227.ckpt')
